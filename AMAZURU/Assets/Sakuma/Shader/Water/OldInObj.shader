    ...
    SubShader
    {
        ...
        Pass
        {
			...
			fixed4 frag(VertexOutput i) : SV_Target
			{
				...
				finalColor = tex2D(_MainTexture, i.uv);
				...
				//fixed3 diffColor = max(0, dot(normal, -L) * _DiffuseShade + (1 - _DiffuseShade)) * finalColor.rgb;
				//finalColor.rgb = diffColor;
				

				finalColor.rgb=((max(0, dot(normal, lightvec))/2)+0.5f) * finalColor.rgb;
				

				
				finalColor=finalColor*((shadowBf/4)+0.75f);
